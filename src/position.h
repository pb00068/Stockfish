...
struct StateInfo {
  ...
  Bitboard   checkersBB;
  StateInfo* previous;
  Bitboard   blockersForKing[COLOR_NB * 2];
...
public:
  ...
  Bitboard blockers_for_king(int i) const;
  Bitboard check_squares(PieceType pt) const;
  Bitboard pinners(int i) const;

  // Attacks to/from a given square
  Bitboard attackers_to(Square s) const;
  Bitboard attackers_to(Square s, Bitboard occupied) const;
  void slider_blockers(Color Us) const;
...
inline Bitboard Position::blockers_for_king(int i) const {
  return st->blockersForKing[i];
}

inline Bitboard Position::pinners(int  i) const {
  return st->pinners[i];
}
