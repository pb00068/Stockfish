...
namespace {
  ...
  void update_continuation_histories(Stack* ss, Piece pc, Square to, int bonus, bool allowNegativeReinforcement);
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
                            ...
                            : (tte->bound() & BOUND_UPPER)))
    {
        ...
        if (ttMove)
        {
            ...
            // Don't apply when already negative
            else if (!ttCapture)
            {
                int penalty = -stat_bonus(depth);
                if (thisThread->mainHistory[us][from_to(ttMove)] >= 0)
...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
      ...
      if (doFullDepthSearch)
      {
          ...
          if (didLMR && !captureOrPromotion)
          {
              ...
              update_continuation_histories(ss, movedPiece, to_sq(move), bonus, bonus > 0);
          }
      }
    ...
    */
             ...
             && !priorCapture)
    {
        ...
        update_continuation_histories(ss-1, pos.piece_on(prevSq), prevSq, stat_bonus(depth) * (1 + extraBonus), true);
    }
  ...
  void update_all_stats(const Position& pos, Stack* ss, Move bestMove, Value bestValue, Value beta, Square prevSq,
    ...
    if (!pos.capture_or_promotion(bestMove))
    {
        ...
        for (int i = 0; i < quietCount; ++i)
        {
            ...
            update_continuation_histories(ss, pos.moved_piece(quietsSearched[i]), to_sq(quietsSearched[i]), -bonus2, true);
        }
    }
    else
    ...
    if (   ((ss-1)->moveCount == 1 + (ss-1)->ttHit || ((ss-1)->currentMove == (ss-1)->killers[0]))
        && !pos.captured_piece())
            update_continuation_histories(ss-1, pos.piece_on(prevSq), prevSq, -bonus1, true);
  ...
  void update_continuation_histories(Stack* ss, Piece pc, Square to, int bonus, bool allowNegativeReinforcement) {
    ...
    for (int i : {1, 2, 4, 6})
    {
        ...
        if (ss->inCheck && i > 2)
            break;
        if (is_ok((ss-i)->currentMove) && allowNegativeReinforcement)
            (*(ss-i)->continuationHistory)[pc][to] << bonus;
        else if (is_ok((ss-i)->currentMove) && !allowNegativeReinforcement && (*(ss-i)->continuationHistory)[pc][to] >= 0) {
            assert(bonus < 0);
            (*(ss-i)->continuationHistory)[pc][to] << bonus;
        }
    }
  ...
  void update_quiet_stats(const Position& pos, Stack* ss, Move move, int bonus) {
    ...
    update_continuation_histories(ss, pos.moved_piece(move), to_sq(move), bonus, true);
