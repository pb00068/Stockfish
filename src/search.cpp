...
moves_loop: // When in check, search starts from here
  ...
  void update_all_stats(const Position& pos, Stack* ss, Move bestMove, Value bestValue, Value beta, Square prevSq,
    ...
    else
    {
        ...
        // decrease also quiet moves when move was scored among 'bad captures' but is promotion or quite SEE positive capture in current pos
        // without this good capture/promo this node would probably have become an all-node
        if (captureCount < 3 && type_of(bestMove) != PROMOTION && (quietCount <= 1 || !pos.see_ge(bestMove, KnightValueMg - PawnValueMg)))
            quietCount = 0;
    }
