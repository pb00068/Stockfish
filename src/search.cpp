...
moves_loop: // When in check, search starts here
  ...
  void update_all_stats(const Position& pos, Stack* ss, Move bestMove, Value bestValue, Value beta, Square prevSq,
    ...
    int smallbonus, largebonus, bestMoveBonus;
    Color us = pos.side_to_move();
    Thread* thisThread = pos.this_thread();
    CapturePieceToHistory& captureHistory = thisThread->captureHistory;
    ...
    largebonus = stat_bonus(depth + 1);
    smallbonus = std::min(largebonus, stat_bonus(depth));
    bestMoveBonus = bestValue > beta + PawnValueMg ? largebonus : smallbonus;
    ...
    if (!pos.capture_or_promotion(bestMove))
    {
        ...
        {
            thisThread->mainHistory[us][from_to(quietsSearched[i])] << -smallbonus;
            update_continuation_histories(ss, pos.moved_piece(quietsSearched[i]), to_sq(quietsSearched[i]), -smallbonus);
        }
    }
    else
