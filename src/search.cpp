...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (!PvNode && ttHit && ttData.is_pv && ss->ply && ss->ply <= LOW_PLY_HISTORY_SIZE && !pos.captured_piece() && (ss-1)->currentMove.is_ok())
        thisThread->lowPlyHistory[ss->ply - 1][(ss-1)->currentMove.from_to()] << 100 * depth;
