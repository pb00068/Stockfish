...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
             ...
             && ttValue < probCutBeta))
    {
        ...
        MovePicker mp(pos, ttMove, probCutBeta - ss->staticEval, &captureHistory, &thisThread->lowPlyHistory, ss->ply);
        int probCutCount = 0;
        bool ttPv = ss->ttPv;
        ss->ttPv = false;
...
moves_loop: // When in check, search starts from here
  ...
  void update_all_stats(const Position& pos, Stack* ss, Move bestMove, Value bestValue, Value beta, Square prevSq,
    ...
    if (depth > 11 && ss->ply < MAX_LPH)
    {
        ...
        if (captured) // make it even
          thisThread->lowPlyHistory[ss->ply][from_to(bestMove)] += thisThread->lowPlyHistory[ss->ply][from_to(bestMove)] % 2;
        else // make it odd
          thisThread->lowPlyHistory[ss->ply][from_to(bestMove)] += (thisThread->lowPlyHistory[ss->ply][from_to(bestMove)] % 2) - (short)1;
    }
