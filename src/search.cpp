    ...
    while ((move = mp.next_move(skipQuiets)) != MOVE_NONE)
    {
          ...
          &&  pos.legal(move))
      {
          ...
          if (value < rBeta)
              extension = ONE_PLY;
               ...
               && !moveCountPruning
               &&  (captureOrPromotion ? pos.see_ge<true>(move, VALUE_ZERO) : pos.see_ge<false>(move, VALUE_ZERO)))
          extension = ONE_PLY;
          ...
          && bestValue > VALUE_MATED_IN_MAX_PLY)
      {
              ...
              && (!pos.advanced_pawn_push(move) || pos.non_pawn_material() >= Value(5000)))
          {
              ...
              if (   lmrDepth < 8
                  && !pos.see_ge<false>(move, Value(-35 * lmrDepth * lmrDepth)))
                  continue;
          }
          else if (    depth < 7 * ONE_PLY
                   && !extension
                   && !pos.see_ge<true>(move, -PawnValueEg * (depth / ONE_PLY)))
                  continue;
      }
          ...
          && (!captureOrPromotion || moveCountPruning))
      {
          ...
          else
          {
              ...
              // hence break make_move().
                       ...
                       && !pos.see_ge<false>(make_move(to_sq(move), from_sq(move)), VALUE_ZERO))
                  r -= 2 * ONE_PLY;
  ...
  Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    while ((move = mp.next_move()) != MOVE_NONE)
    {
          ...
          && !pos.advanced_pawn_push(move))
      {
          ...
          if (futilityBase <= alpha && !pos.see_ge<true>(move, VALUE_ZERO + 1))
          {
      ...
      if (  (!InCheck || evasionPrunable)
          && !pos.see_ge<true>(move, VALUE_ZERO))
          continue;
