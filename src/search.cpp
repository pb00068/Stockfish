...
namespace {
  ...
  void update_quiet_stats(const Position& pos, Stack* ss, Move move, Move* quiets, int quietsCnt, bool excl, int bonus);
        ...
        if (ttMove)
        {
            if (ttValue >= beta)
            {
                if (!pos.capture_or_promotion(ttMove))
                    update_quiet_stats(pos, ss, ttMove, nullptr, 0, false, stat_bonus(depth));
...
moves_loop: // When in check, search starts from here
    ...
    */
    ...
    else if (bestMove)
    {
        ...
        if (!pos.capture_or_promotion(bestMove))
            update_quiet_stats(pos, ss, bestMove, quietsSearched, quietCount, bool(excludedMove),
  ...
  void update_quiet_stats(const Position& pos, Stack* ss, Move move,
                          Move* quiets, int quietsCnt, bool excl, int bonus) {

    if (!excl && ss->killers[0] != move)
    {
