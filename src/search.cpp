...
moves_loop: // When in check, search starts here
  ...
  void update_all_stats(const Position& pos, Stack* ss, Move bestMove, Value bestValue, Value beta, Square prevSq,
    ...
    if (!pos.capture_or_promotion(bestMove))
    {
        ...
        if (type_of(moved_piece) > captured && quietCount > 1 && depth > 3) {
             for (int i = 0; i < quietCount; ++i)
             {
                  thisThread->mainHistory[us][from_to(quietsSearched[i])] << -stat_bonus(depth-1);
                  update_continuation_histories(ss, pos.moved_piece(quietsSearched[i]), to_sq(quietsSearched[i]), -stat_bonus(depth-1));
             }
        }
    }
