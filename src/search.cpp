    ...
    else if (bestMove)
    {
        ...
        if ((ss-1)->moveCount == 1)
        {
        	if (!pos.captured_piece())
                ...
                if (pos.this_thread()->captureHistory[pos.piece_on(prevSq)][prevSq][type_of(pos.captured_piece())] > 0)
                	pos.this_thread()->captureHistory[pos.piece_on(prevSq)][prevSq][type_of(pos.captured_piece())] << -stat_bonus(depth + ONE_PLY);
        	}
        }
    }
