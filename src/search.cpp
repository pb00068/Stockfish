...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    ss->aftercapture = pos.captured_piece();
                            ...
                            : (tte->bound() & BOUND_UPPER)))
    {
        ...
        if (ttMove)
        {
            if (ttValue >= beta)
            {
                ...
                if ((ss-1)->moveCount <= 2 && !ss->aftercapture && !(ss-1)->aftercapture)
        ...
        &&  abs(beta) < VALUE_MATE_IN_MAX_PLY)
    {
        ...
        while (  (move = mp.next_move()) != MOVE_NONE
            ...
            if (move != excludedMove && pos.legal(move))
            {
                ...
                ss->currentMove = move;
                ss->continuationHistory = &thisThread->continuationHistory[inCheck][ss->aftercapture || (ss-1)->aftercapture][pos.moved_piece(move)][to_sq(move)];
...
moves_loop: // When in check, search starts from here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
      ...
      ss->currentMove = move;
      ss->continuationHistory = &thisThread->continuationHistory[inCheck][ss->aftercapture || (ss-1)->aftercapture][movedPiece][to_sq(move)];
    ...
    */
    ...
    else if (bestMove)
    {
        ...
        if (   ((ss-1)->moveCount == 1 || ((ss-1)->currentMove == (ss-1)->killers[0]))
            && !ss->aftercapture && !(ss-1)->aftercapture)
                update_continuation_histories(ss-1, pos.piece_on(prevSq), prevSq, -stat_bonus(depth + 1));
