...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (depth <= 0)
    {
        Value qv =  qsearch<PvNode ? PV : NonPV>(pos, ss, alpha, beta, false);
        if (mate_distance(qv) > 0)
           depth = PvNode ? 3 : 2;// mate scores on PV must be verfied by normal search
        else
          return qv;
    }
...
template<NodeType nodeType>
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta, bool findMate) {
    ...
    Value unadjustedStaticEval = VALUE_NONE;
    if (ss->inCheck || findMate || (PvNode && mate_distance(ttData.value)))
    ...
    Square prevSq = ((ss - 1)->currentMove).is_ok() ? ((ss - 1)->currentMove).to_sq() : SQ_NONE;
    bool catchMate = ((is_valid(ttData.value) && mate_distance(ttData.value)) || findMate) && !ss->inCheck;
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        // Step 6. Pruning
        if (!is_loss(bestValue) && !catchMate)
        {
        ...
        value = -qsearch<nodeType>(pos, ss + 1, -beta, -alpha, PvNode && ttHit && mate_distance(ttData.value) && is_loss(ttData.value));
        undo_move(pos, move);
