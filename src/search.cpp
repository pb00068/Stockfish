...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
                            ...
                            : (tte->bound() & BOUND_UPPER)))
    {
        ...
        if (ttMove)
        {
            ...
            else if (!pos.capture_or_promotion(ttMove))
            {
                ...
                thisThread->mainHistory[us][from_to(ttMove)][(pos.game_ply()/8 + 0) % 6] << penalty;
                thisThread->mainHistory[us][from_to(ttMove)][(pos.game_ply()/8 + 1) % 6] << penalty/2;
                thisThread->mainHistory[us][from_to(ttMove)][(pos.game_ply()/8 - 1) % 6] << penalty/2;
    ...
    if (is_ok((ss-1)->currentMove) && !(ss-1)->inCheck && !priorCapture)
    {
        ...
        thisThread->mainHistory[~us][from_to((ss-1)->currentMove)][(pos.game_ply()/8 + 0) % 6] << bonus;
        thisThread->mainHistory[~us][from_to((ss-1)->currentMove)][(pos.game_ply()/8 - 1) % 6] << bonus/2;
        thisThread->mainHistory[~us][from_to((ss-1)->currentMove)][(pos.game_ply()/8 + 1) % 6] << bonus/2;
    }
...
moves_loop: // When in check, search starts from here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
              ...
              || thisThread->ttHitAverage < 432 * TtHitAverageResolution * TtHitAverageWindow / 1024))
      {
          ...
          else
          {
              ...
              ss->statScore =  thisThread->mainHistory[us][from_to(move)][(pos.game_ply()/8) % 6]
                             + (*contHist[0])[movedPiece][to_sq(move)]
                             + (*contHist[1])[movedPiece][to_sq(move)]
                             + (*contHist[3])[movedPiece][to_sq(move)]
                             - 5337;
              ...
              if (ss->inCheck)
                  r -= (thisThread->mainHistory[us][from_to(move)][(pos.game_ply()/8) % 6]
  ...
  void update_all_stats(const Position& pos, Stack* ss, Move bestMove, Value bestValue, Value beta, Square prevSq,
    ...
    if (!pos.capture_or_promotion(bestMove))
    {
        ...
        int j = (pos.game_ply()/8 - 0) % 6;
        int k = (pos.game_ply()/8 + 1) % 6;
        int l = (pos.game_ply()/8 - 1) % 6;
  ...
  void update_quiet_stats(const Position& pos, Stack* ss, Move move, int bonus, int depth) {
    ...
    Color us = pos.side_to_move();
    Thread* thisThread = pos.this_thread();
    int i = (pos.game_ply()/8 + 0) % 6;
    int j = (pos.game_ply()/8 + 1) % 6;
    int k = (pos.game_ply()/8 - 1) % 6;
