...
moves_loop: // When in check, search starts here
    ...
    if (!pos.capture(bestMove))
    {
        ...
        for (int i = 0; i < quietCount; ++i)
        {
            ...
            if (depth > 5 && type_of(pos.moved_piece(quietsSearched[i])) == PAWN) {
                  ...
                  if (cat & 8)
                    thisThread->pawnHistory[us][3][to_sq(quietsSearched[i])] << -bonus2;
                  if (cat & 16)
                    thisThread->pawnHistory[us][4][to_sq(quietsSearched[i])] << -bonus2;
                  if (!cat)
                     thisThread->pawnHistory[us][5][to_sq(quietsSearched[i])] << -bonus2;
            }
  ...
  void update_quiet_stats(const Position& pos, Stack* ss, Move move, int bonus, Depth depth) {
    ...
    if (depth > 5 && type_of(pos.moved_piece(move)) == PAWN) {
       ...
       if (cat & 8)
         thisThread->pawnHistory[us][3][to_sq(move)] << bonus;
       if (cat & 16)
         thisThread->pawnHistory[us][4][to_sq(move)] << bonus;
       if (!cat)
         thisThread->pawnHistory[us][5][to_sq(move)] << bonus;
    }
