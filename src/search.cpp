...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (ss->inCheck)
    {
        ...
        // excludeMove implies that we had a ttHit on the containing non-excluded search with ss->static-eval taken from TT
        // However Static evals from the TT aren't good enough (-13 elo), presumably due to changing optimism context
        // Recalculate value with current optimism, without saving it into TT, without updating thread avgComplexity
