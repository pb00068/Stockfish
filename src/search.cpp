...
moves_loop: // When in check, search starts from here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
              ...
              || givesCheck)
          {
              ...
              if (   !givesCheck
                  && lmrDepth < 1
                  && captureHistory[movedPiece][to_sq(move)][reductPieceType(type_of(pos.piece_on(to_sq(move))))] < 0)
                  continue;
      ...
      if (    depth >= 3
          ...
          && (  !captureOrPromotion
              ...
              || cutNode
              || (!PvNode && !formerPv && captureHistory[movedPiece][to_sq(move)][reductPieceType(type_of(pos.captured_piece()))] < 4506)
  ...
  void update_all_stats(const Position& pos, Stack* ss, Move bestMove, Value bestValue, Value beta, Square prevSq,
    ...
    if (!pos.capture_or_promotion(bestMove))
    {
        ...
        captureHistory[moved_piece][to_sq(bestMove)][reductPieceType(captured)] << bonus1;
    ...
    for (int i = 0; i < captureCount; ++i)
    {
        ...
        captureHistory[moved_piece][to_sq(capturesSearched[i])][reductPieceType(captured)] << -bonus1;
    }
  }
