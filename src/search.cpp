...
namespace {
  ...
  void update_continuation_histories(Stack* ss, Piece pc, Square to, int bonus, bool from);
...
moves_loop: // When in check, search starts here
    ...
    const PieceToHistory* contHist[] = { (ss-1)->continuationHistory, (ss-2)->continuationHistory,
                                         (ss-1)->continuationHistoryFrom, (ss-4)->continuationHistory,
  ...
  void update_continuation_histories(Stack* ss, Piece pc, Square to, int bonus, bool from) {
    ...
    for (int i : {1, 2, 4, 6})
    {
        ...
        if (is_ok((ss-i)->currentMove))
        {
            ...
            if (i==1 && from)
               (*(ss-i)->continuationHistoryFrom)[pc][to] << (bonus > 0 ? bonus : bonus/16);
        }
    }
  }
  ...
  void update_quiet_stats(const Position& pos, Stack* ss, Move move, int bonus, int depth) {
    ...
    bool from = (ss->killers[0] != move && from_sq(move) == from_sq(ss->killers[0])) ||
                (ss->killers[1] != move && from_sq(move) == from_sq(ss->killers[1]));
    // Update killers
    ...
    update_continuation_histories(ss, pos.moved_piece(move), to_sq(move), bonus, from);
