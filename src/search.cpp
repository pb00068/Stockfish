...
namespace {
  ...
  void update_quiet_stats(const Position& pos, Stack* ss, Move move, Move* quiets, int quietsCnt, int bonus, int penalty);
...
moves_loop: // When in check, search starts from here
    ...
    else if (bestMove)
    {
        ...
        if (!pos.capture_or_promotion(bestMove))
            update_quiet_stats(pos, ss, bestMove, quietsSearched, quietCount,
                               stat_bonus(depth + (bestValue > beta + PawnValueMg ? ONE_PLY : DEPTH_ZERO)), -stat_bonus(depth));
        else
  ...
  void update_quiet_stats(const Position& pos, Stack* ss, Move move,
                          Move* quiets, int quietsCnt, int bonus, int penalty) {
    ...
    {
        thisThread->mainHistory[us][from_to(quiets[i])] << penalty;
        update_continuation_histories(ss, pos.moved_piece(quiets[i]), to_sq(quiets[i]), penalty);
    }
  }
