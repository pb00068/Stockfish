...
namespace {
  ...
  void update_quiet_stats(const Position& pos, Stack* ss, Move move, int bonus, int depth, Square from);
  void update_all_stats(const Position& pos, Stack* ss, Move bestMove, Value bestValue, Value beta, Square prevSq,
                        Move* quietsSearched, int quietCount, Move* capturesSearched, int captureCount, Depth depth, Square from);
        ...
        if (ttMove)
        {
            if (ttValue >= beta)
            {
                ...
                if (!pos.capture_or_promotion(ttMove))
                    update_quiet_stats(pos, ss, ttMove, stat_bonus(depth), depth, SQ_NONE);
...
moves_loop: // When in check, search starts here
  ...
  void update_all_stats(const Position& pos, Stack* ss, Move bestMove, Value bestValue, Value beta, Square prevSq,
    ...
    if (!pos.capture_or_promotion(bestMove))
    {
        ...
        update_quiet_stats(pos, ss, bestMove, bonus2, depth, from);
  ...
  void update_quiet_stats(const Position& pos, Stack* ss, Move move, int bonus, int depth, Square fromSq) {

    Thread* thisThread = pos.this_thread();
    Color us = pos.side_to_move();
    if (fromSq == from_sq(move) ||
        (move != ss->killers[0] && from_sq(ss->killers[0]) == from_sq(move)) ||
        (move != ss->killers[1] && from_sq(ss->killers[1]) == from_sq(move)))
        thisThread->clearingHistory[us][from_sq(move)] << bonus;
