...
#define S(mg, eg) make_score(mg, eg)
  ...
  constexpr Score BishopXRayPawns     = S(  6,  5);
    ...
    for (Square s = *pl; s != SQ_NONE; s = *++pl)
    {
        ...
        if (Pt == BISHOP || Pt == KNIGHT)
        {
            ...
            if (Pt == BISHOP)
            {
                ...
                // Penalty for all pawn protected enemy pawns x-rayed
                score -= BishopXRayPawns * popcount(PseudoAttacks[BISHOP][s] & pos.pieces(Them, PAWN) & attackedBy[Them][PAWN]);
