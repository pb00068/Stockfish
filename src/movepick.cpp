...
void MovePicker::score<QUIETS>() {
  ...
  for (auto& m : *this)
      m.value =      history[pos.moved_piece(m)][to_sq(m)]
               ...
               + fromTo.get(c, m)
               + ((is_ok((ss-2)->currentMove) && is_ok((ss-1)->currentMove) && !pos.captured_piece_type()) ? pos.this_thread()->moveSeq.get(to_sq((ss-2)->currentMove), to_sq((ss-1)->currentMove), to_sq(m)) : VALUE_ZERO);
}
