...
void MovePicker::score<QUIETS>() {
  ...
  const SquareSquareStats& fromTo = pos.this_thread()->fromTo;
  const SquareSquareStats& nullFromTo = pos.this_thread()->fromTo;

  const CounterMoveStats* cm = (ss-1)->counterMoves;
  const CounterMoveStats* fm = (ss-1)->currentMove != MOVE_NULL ? (ss-2)->counterMoves : nullptr;
  ...
  for (auto& m : *this)
      m.value =      history[pos.moved_piece(m)][to_sq(m)]
               ...
               + fromTo.get(c, from_sq(m), to_sq(m))
               + ((ss-1)->currentMove == MOVE_NULL ? nullFromTo.get(c, to_sq((ss-2)->currentMove), to_sq(m)) : VALUE_ZERO);
}
...
void MovePicker::score<EVASIONS>() {
  ...
  for (auto& m : *this)
      if ((see = pos.see_sign(m)) < VALUE_ZERO)
          ...
          m.value = history[pos.moved_piece(m)][to_sq(m)] + fromTo.get(c, from_sq(m), to_sq(m));
}
