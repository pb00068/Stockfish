...
MovePicker::MovePicker(const Position& p, Move ttm, Depth d,
                       const HistoryStats& h, Square s, Search::Stack* st)
           : pos(p), history(h), ss(st) {
...
MovePicker::MovePicker(const Position& p, Move ttm, const HistoryStats& h, Value th,  Search::Stack* s)
           : pos(p), history(h), ss(s), threshold(th)  {
  ...
  ttMove =   ttm
          ...
          && pos.capture(ttm)
          && pos.see(ttm, ss->notpinneds) > threshold ? ttm : MOVE_NONE;
...
void MovePicker::score<EVASIONS>() {
  ...
  for (auto& m : *this)
      if ((see = pos.see_sign(m, ss->notpinneds)) < VALUE_ZERO)
...
Move MovePicker::next_move() {
  ...
  while (true)
  {
      ...
      case GOOD_CAPTURES:
          ...
          if (move != ttMove)
          {
              if (pos.see_sign(move, ss->notpinneds) >= VALUE_ZERO)
                  return move;
      ...
      case PROBCUT_CAPTURES:
           move = pick_best(cur++, endMoves);
           if (move != ttMove && pos.see(move, ss->notpinneds) > threshold)
               return move;
           break;
