...
Move MovePicker::next_move(bool skipQuiets) {
  ...
  case CAPT_KILLER:
  	  ...
  	  {
  		  int16_t ttval = (*captureHistory)[pos.moved_piece(ttMove)][to_sq(ttMove)][type_of(pos.piece_on(to_sq(ttMove)))];
  	      move = killers[2];
  		  if (move && move != ttMove && pos.pseudo_legal(move) && pos.capture_or_promotion(move) &&
  			(*captureHistory)[pos.moved_piece(move)][to_sq(move)][type_of(pos.piece_on(to_sq(move)))] > ttval && pos.see_ge(move))
  			  return move;
  		  else {
  			 move = killers[2] = killers[3];
  			 if (move && move != ttMove && pos.pseudo_legal(move) && pos.capture_or_promotion(move) &&
  			  	(*captureHistory)[pos.moved_piece(move)][to_sq(move)][type_of(pos.piece_on(to_sq(move)))] > ttval && pos.see_ge(move))
  			  return move;
